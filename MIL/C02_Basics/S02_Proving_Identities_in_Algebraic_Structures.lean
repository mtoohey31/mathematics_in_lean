...
theorem self_sub (a : R) : a - a = 0 := by
  rw [sub_eq_add_neg, <- zero_add (a + -a), <- add_assoc, add_neg_cancel_right]
...
theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  rw [<- one_mul (b⁻¹), <- mul_left_inv (a * b), <- mul_assoc, mul_assoc,
      mul_assoc, <- mul_assoc b, mul_right_inv, one_mul, mul_assoc,
