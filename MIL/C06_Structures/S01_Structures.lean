...
def weightedAverage (lambda : Real) (lambda_nonneg : 0 ≤ lambda) (lambda_le : lambda ≤ 1)
  ...
  sum_eq := by
    calc
      _ = lambda * (a.x + a.y + a.z) + (1 - lambda) * (b.x + b.y + b.z) := by linarith
      _ = 1 := by
...
def weightedAverage (n : ℕ) (lambda : Real) (lambda_nonneg : 0 ≤ lambda) (lambda_le : lambda ≤ 1)
  ...
  NonNeg := by
    intro i
    dsimp
    apply add_nonneg (mul_nonneg lambda_nonneg (a.NonNeg i))
    . apply mul_nonneg (sub_nonneg.mpr lambda_le) (b.NonNeg i)
  sum_eq_one := by
